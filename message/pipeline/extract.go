...
func newExtracter(c *Config) (x *extracter, err error) {
	...
	x.ipkgs, err = loadPackages(&x.conf, c.Packages)
	if err != nil {
		return nil, wrap(err, "")
	}

	x.prog, x.pkgs = ssautil.AllPackages(x.ipkgs, ssa.GlobalDebug|ssa.BareInits)
	x.prog.Build()
...
func (x *extracter) seedEndpoints() error {
	pkg := x.prog.ImportedPackage("golang.org/x/text/message")
	if pkg == nil {
	...
	if data.callArgsStart() >= 0 {
		...
		for i, arg := range args {
			...
			arguments = append(arguments, argument{
				...
				Position:       posString(&x.conf, info, arg.Pos()),
...
func (px packageExtracter) addMessage(
	...
	for p.SetFormat(trimmed); p.Scan(); {
		...
		case fmtparser.StatusBadArgNum, fmtparser.StatusMissingArg:
			arg = &argument{
				ArgNum:   p.ArgNum,
				Position: posString(&x.conf, px.info, pos),
			}
...
func posString(conf *packages.Config, pkg *packages.Package, pos token.Pos) string {
	p := pkg.Fset.Position(pos)
	file := fmt.Sprintf("%s:%d:%d", filepath.Base(p.Filename), p.Line, p.Column)
	return filepath.Join(pkg.PkgPath, file)
}
